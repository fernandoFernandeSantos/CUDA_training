...
__global__ void sgemm_kernel_N_N_64_16_16_16_4_special(float *C, const float *A,
		const float *B, int m, int n, int k, int lda, int ldb, int ldc,
...
void sgemm_N_N_64_16_16_16_4_special(cudaStream_t stream, float *C, const float *A,
		const float *B, int32_t m, int32_t n, int32_t k, int32_t lda,
		int32_t ldb, int32_t ldc, float alpha, float beta) {
	dim3 threads(16, 4);
	dim3 grid(m / 64, n / 16);
	sgemm_kernel_N_N_64_16_16_16_4_special<<<grid, threads, 0, stream>>>(
